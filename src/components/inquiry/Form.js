            ...
            if (result.isConfirmed) {
                fetch('/api/inquiry', {
                    ...
                    if (data?.message === 'OK') {
                        ...
                        if (msg.indexOf("is null") > -1) {
                            text = `<div>입력하지 않은 내용이 있습니다.</div><div>다시 입력해주세요.</div>`
                        } else if (msg.indexOf("is too long") > -1) {
                            text = `<div>입력하신 내용이 너무 깁니다.</div><div>다시 입력해주세요.</div>`
                        } else if (msg.indexOf("is invalid") > -1) {
                            text = `<div>유효하지않은 ${msg.substring(0, msg.indexOf("is invalid"))} 형식입니다.</div><div>다시 입력해주세요.</div>`
                        } else {
