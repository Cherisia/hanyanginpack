            ...
            <div className="max-w-3xl mx-auto px-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {stats.map((s, i) => (
                        <div
                            key={s.label}
                            className={`reveal reveal-d${i + 1} text-center bg-sky-50 border border-sky-100 rounded-2xl px-4 py-6 transition-[transform,box-shadow,background-color] duration-700 hover:-translate-y-1.5 hover:shadow-lg hover:shadow-sky-100 hover:bg-sky-200 cursor-default`}
                        >
                            <div className="text-2xl font-black text-gray-900">
                                {s.value}<span className="text-sky-500 text-lg">{s.unit}</span>
                            </div>
